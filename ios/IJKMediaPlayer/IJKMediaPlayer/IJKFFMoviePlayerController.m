...
- (instancetype)initWithContentURLString:(NSString *)aUrlString withOptions:(IJKFFOptions *)options
{
    ...
    if (self) {
        ...
        // DXLiveIOStatRegister(DXLiveIOStatDebugCallback);
        // DXLiveIOStatCompleteRegister(DXLiveIOStatCompleteDebugCallback);
        ...
        // init player
        _mediaPlayer = ijkmp_ios_create(dxmedia_player_msg_loop);
...
// as an example
void DXLiveIOStatDebugCallback(const char *url, int type, int bytes)
{
    ...
    if (s_ff_io_stat_bytes < s_ff_io_stat_check_points || s_ff_io_stat_bytes > s_ff_io_stat_check_points + FFP_IO_STAT_STEP) {
        s_ff_io_stat_check_points = s_ff_io_stat_bytes;
        NSLog(@"io-stat: %s, +%d = %"PRId64"\n", url, bytes, s_ff_io_stat_bytes);
    }
}

void DXLiveIOStatRegister(void (*cb)(const char *url, int type, int bytes))
{
    ijkmp_io_stat_register(cb);
}

void DXLiveIOStatCompleteDebugCallback(const char *url, int64_t read_bytes, int64_t total_size, int64_t elpased_time, int64_t total_duration)
{
    if (!url)
        return;
...
void DXLiveIOStatCompleteRegister(void (*cb)(const char *url, int64_t read_bytes, int64_t total_size, int64_t elpased_time, int64_t total_duration))
{
...
- (void)shutdownClose:(IJKFFMoviePlayerController *) mySelf
{
    ...
    self.segmentOpenDelegate = nil;
    self.tcpOpenDelegate = nil;
    self.httpOpenDelegate = nil;
    self.liveOpenDelegate = nil;
    self.nativeInvokeDelegate = nil;
...
- (void)changeNaturalSize
{
    ...
    if (self->_naturalSize.width > 0 && self->_naturalSize.height > 0) {
        [[NSNotificationCenter defaultCenter] postNotificationName:IJKMPMovieNaturalSizeAvailableNotification object:self];
    }
}
...
- (void)postEvent:(IJKFFMoviePlayerMessage *)msg
{
    ...
    switch (avmsg->what) {
        ...
        case FFP_MSG_ERROR: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:IJKMPMoviePlayerPlaybackStateDidChangeNotification object:self];
            [[NSNotificationCenter defaultCenter] postNotificationName:IJKMPMoviePlayerPlaybackDidFinishNotification object:self userInfo:@{ IJKMPMoviePlayerPlaybackDidFinishReasonUserInfoKey: @(IJKMPMovieFinishReasonPlaybackError),
                                                                                                                                             @"error": @(avmsg->arg1)}];
            break;
        }
        ...
        case FFP_MSG_COMPLETED: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:IJKMPMoviePlayerPlaybackStateDidChangeNotification object:self];
            [[NSNotificationCenter defaultCenter] postNotificationName:IJKMPMoviePlayerPlaybackDidFinishNotification object:self userInfo:@{IJKMPMoviePlayerPlaybackDidFinishReasonUserInfoKey: @(IJKMPMovieFinishReasonPlaybackEnded)}];
            break;
        }
        ...
        case FFP_MSG_SEEK_COMPLETE: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:IJKMPMoviePlayerDidSeekCompleteNotification object:self userInfo:@{IJKMPMoviePlayerDidSeekCompleteTargetKey : @(avmsg->arg1), IJKMPMoviePlayerDidSeekCompleteErrorKey : @(avmsg->arg2)}];
            _seeking = NO;
            break;
        }
        case FFP_MSG_VIDEO_DECODER_OPEN: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:IJKMPMoviePlayerVideoDecoderOpenNotification object:self];
            break;
        }
        case FFP_MSG_VIDEO_RENDERING_START: {
            NSLog(@"FFP_MSG_VIDEO_RENDERING_START:\n");
            _monitor.firstVideoFrameLatency = (int64_t)SDL_GetTickHR() - _monitor.prepareStartTick;
            [[NSNotificationCenter defaultCenter] postNotificationName:IJKMPMoviePlayerFirstVideoFrameRenderedNotification object:self];
            break;
        }
        case FFP_MSG_AUDIO_RENDERING_START: {
            NSLog(@"FFP_MSG_AUDIO_RENDERING_START:\n");
            [[NSNotificationCenter defaultCenter] postNotificationName:IJKMPMoviePlayerFirstAudioFrameRenderedNotification object:self];
            break;
        }
        default:
...
int dxmedia_player_msg_loop(void* arg)
{
    @autoreleasepool {
...
static int onInjectIOControl(IJKFFMoviePlayerController *mpc, id<IJKMediaUrlOpenDelegate> delegate, int type, void *data, size_t data_size)
{
    ...
    IJKMediaUrlOpenData *openData = [[IJKMediaUrlOpenData alloc] initWithUrl:urlString
...
- (void)setAllowsMediaAirPlay:(BOOL)allowsMediaAirPlay
{
    if (!self)
        return;
    _allowsMediaAirPlay = allowsMediaAirPlay;
}
...
{
    [_notificationManager addObserver:self selector:@selector(audioSessionInterrupt:)
